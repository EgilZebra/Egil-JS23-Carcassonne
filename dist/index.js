...
if (canvas) {
    ...
    if (ctx) {
        ...
        */
        ...
        let startMenytoggle = true;
        let endMenuToggle = false;
        let dragMapX = 0;
        let dragMapY = 0;
        ...
        */
        ...
        const mouseClickStart = (e) => {
            // Handle the mouse-clicks on the StartMenu
            const Mouse = mousePlacement(e, canvas, GameBoardSize.x, deltaDragX, GameBoardSize.y, deltaDragY);
            // Select the color for player 1
            if (Mouse.scaledX > 704 && Mouse.scaledX < 784 && Mouse.scaledY > 380 && Mouse.scaledY < 455) {
                console.log('blue');
                allPlayersStats[0].color = (allPlayersStats[1].color !== playerColors[0]) ? playerColors[0] : allPlayersStats[0].color;
            }
            if (Mouse.scaledX > 684 + 100 && Mouse.scaledX < 684 + 100 + 80 && Mouse.scaledY > 380 && Mouse.scaledY < 455) {
                console.log('green');
                allPlayersStats[0].color = (allPlayersStats[1].color !== playerColors[1]) ? playerColors[1] : allPlayersStats[0].color;
            }
            if (Mouse.scaledX > 764 + 100 && Mouse.scaledX < 764 + 100 + 80 && Mouse.scaledY > 380 && Mouse.scaledY < 455) {
                console.log('yellow');
                allPlayersStats[0].color = (allPlayersStats[1].color !== playerColors[2]) ? playerColors[2] : allPlayersStats[0].color;
            }
            if (Mouse.scaledX > 844 + 100 && Mouse.scaledX < 844 + 100 + 80 && Mouse.scaledY > 380 && Mouse.scaledY < 455) {
                console.log('red');
                allPlayersStats[0].color = (allPlayersStats[1].color !== playerColors[3]) ? playerColors[3] : allPlayersStats[0].color;
            }
            if (Mouse.scaledX > 924 + 100 && Mouse.scaledX < 924 + 100 + 80 && Mouse.scaledY > 380 && Mouse.scaledY < 455) {
                console.log('black');
                allPlayersStats[0].color = (allPlayersStats[1].color !== playerColors[4]) ? playerColors[4] : allPlayersStats[0].color;
            }
            //  Select the color for player 2
            if (Mouse.scaledX > 704 && Mouse.scaledX < 704 + 80 && Mouse.scaledY > 90 + 490 && Mouse.scaledY < 90 + 490 + 75) {
                console.log('blue');
                allPlayersStats[1].color = (allPlayersStats[0].color !== playerColors[0]) ? playerColors[0] : allPlayersStats[1].color;
            }
            if (Mouse.scaledX > 684 + 100 && Mouse.scaledX < 684 + 100 + 80 && Mouse.scaledY > 90 + 490 && Mouse.scaledY < 90 + 490 + 75) {
                console.log('green');
                allPlayersStats[1].color = (allPlayersStats[0].color !== playerColors[1]) ? playerColors[1] : allPlayersStats[1].color;
            }
            if (Mouse.scaledX > 764 + 100 && Mouse.scaledX < 764 + 100 + 80 && Mouse.scaledY > 90 + 490 && Mouse.scaledY < 90 + 490 + 75) {
                console.log('yellow');
                allPlayersStats[1].color = (allPlayersStats[0].color !== playerColors[2]) ? playerColors[2] : allPlayersStats[1].color;
            }
            if (Mouse.scaledX > 844 + 100 && Mouse.scaledX < 844 + 100 + 80 && Mouse.scaledY > 90 + 490 && Mouse.scaledY < 90 + 490 + 75) {
                console.log('red');
                allPlayersStats[1].color = (allPlayersStats[0].color !== playerColors[3]) ? playerColors[3] : allPlayersStats[1].color;
            }
            if (Mouse.scaledX > 924 + 100 && Mouse.scaledX < 924 + 100 + 80 && Mouse.scaledY > 90 + 490 && Mouse.scaledY < 90 + 490 + 75) {
                console.log('black');
                allPlayersStats[1].color = (allPlayersStats[0].color !== playerColors[4]) ? playerColors[4] : allPlayersStats[1].color;
            }
            // Start The Game.
        ...
        function animate() {
            if (ctx) {
                ...
                if (startMenytoggle) {
                    ...
                    drawGameBoard(ctx, gameMap, placedMeeples, GameBoardSize, GameBoardSize.tileSize, GameBoardSize.tileSize, deltaDragX, deltaDragY, MeepleInfo);
                    // Generating the remaining meeples for each player.
                    drawMeeples(ctx, allPlayersStats[currentPlayer].remainingMeeples, playerColors.indexOf(allPlayersStats[currentPlayer].color), meepleImage, GameBoardSize.meepleSize);
                    // Update the GameBoard to show which tiles are avalible for play this turn.
                    drawPlayableTiles(ctx, playableTiles, GameBoardSize, playableTilesBorderWidth, drawNewTile, GameBoardSize.tileSize, GameBoardSize.tileSize, moveTiletoggle, checkAjecentTiles, deltaDragX, deltaDragY, gameMap);
